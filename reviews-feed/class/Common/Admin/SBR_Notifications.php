...
// Note: Legacy file with complex notification handling. Refactoring planned.
...
class SBR_Notifications extends ServiceProvider
{
	...
	public function verify_active($notifications)
	{
		...
		foreach ($notifications as $key => $notification) {
			if (
				( ! empty($notification['start']) && sbr_get_current_time() < strtotime($notification['start']) )
	...
	public function get_notifcation_info_json()
	{
		...
		$is_review_notice = ! empty($notifications[0]) && ! empty($notifications[0]['id']);

		// CUSTOM MODIFICATION: Allow all notifications, not just review notices
		// Original code only displayed if first notification was a review notice
		// Commented out to allow custom notifications to display
	...
	public function output()
	{
		...
		foreach ($notifications as $notification) {
			...
			if (empty($notification['image'])) {
				...
				} elseif ($notification['image'] === 'review' || $notification['image'] === 'discount') {
					$image_html = sprintf(
						'<div class="bell"><img src="%s" alt="notice">',
